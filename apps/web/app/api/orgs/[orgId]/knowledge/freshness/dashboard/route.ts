import { getKnowledgeFreshnessDashboard } from "@internalwiki/db";
...
export async function GET(
  ...
  const rate = await checkRateLimit({
    key: `${session.organizationId}:${session.userId}:knowledge_freshness_dashboard`,
    windowMs: 60_000,
    maxRequests: 120
  });
  if (!rate.allowed) {
  ...
  const dashboard = await getKnowledgeFreshnessDashboard(orgId);
  return jsonOk({ dashboard }, withRequestId(requestId));
}
