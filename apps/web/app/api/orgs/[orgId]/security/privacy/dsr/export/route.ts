...
export async function POST(
  ...
  const rate = await checkRateLimit({
    key: `${session.organizationId}:${session.userId}:privacy_dsr_export`,
    windowMs: 60_000,
    maxRequests: 15
  });
  if (!rate.allowed) {
  ...
  const parsed = dsrExportSchema.safeParse(await request.json().catch(() => ({})));
  if (!parsed.success) {
  ...
  const exportResult = await createPrivacyExportRequest({
    organizationId: orgId,
    subjectUserId,
  ...
  await writeAuditEvent({
    organizationId: orgId,
    actorId: session.userId,
    eventType: "security.privacy.dsr_export",
    entityType: "privacy_request",
    entityId: exportResult.request.id,
    payload: {
      subjectUserId,
      status: exportResult.request.status
    }
  });
